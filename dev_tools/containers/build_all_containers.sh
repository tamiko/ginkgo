...
hpccm --recipe ginkgo-nocuda-base.py --userarg gnu=8 llvm=7 papi=True metis=True mpi=True > gko-nocuda-gnu8-llvm70.baseimage
list=('gko-nocuda-gnu8-llvm70.baseimage')
hpccm --recipe ginkgo-nocuda-base.py --userarg gnu=9 llvm=8 papi=True metis=True mpi=True > gko-nocuda-gnu9-llvm8.baseimage
list+=('gko-nocuda-gnu9-llvm8.baseimage')
if [ "$HOSTNAME" = "amdci" ]; then
  list+=('gko-amd-gnu7-llvm60.baseimage')
else
  hpccm --recipe ginkgo-cuda-base.py --userarg cuda=10.1 gnu=8 llvm=7 metis=True mpi=True > gko-cuda101-gnu8-llvm70.baseimage
